...
class ChildRegistrationViewModel @Inject constructor(
    ...
    private val updateChildVisitUseCase: UpdateChildVisitUseCase, // ADDED
    private val repository: ChildRepository, // ADDED

) : ViewModel() {

    // State utama untuk data formulir
    private val _currentChild = MutableLiveData(ChildRegistrationData())
    val currentChild: LiveData<ChildRegistrationData> = _currentChild
    ...
    fun loadVisitForEditing(visitId: Int) = viewModelScope.launch {
        ...
        if (visitEntity != null) {
            ...
            if (motherEntity != null) {
                ...
                _currentChild.value = motherEntity.toRegistrationData()
                _currentChildVisit.value = visitEntity.toData()
            } else {
    ...
    fun resetForm() {
        _currentChild.value = ChildRegistrationData()
        _currentChildVisit.value = ChildVisitData()
    ...
    private fun loadUserLocationDetails() {
        viewModelScope.launch {
            ...
            if (kelurahanId != null) {
                ...
                if (_currentChild.value?.name.isNullOrEmpty()) {
    ...
    fun updateChildData(
        ...
        val currentData = _currentChild.value ?: ChildRegistrationData()
        ...
        _currentChild.value = updatedData
    }
    ...
    fun updateChildVisitData(
        ...
        val currentData = _currentChildVisit.value ?: ChildVisitData()
        val updatedData = currentData.copy(
        ...
        _currentChildVisit.value = updatedData
    }
    ...
    fun startNewVisitForExistingMother(child: ChildEntity) {
    ...
    fun saveAllData() = viewModelScope.launch {
        ...
        val motherData = _currentChild.value
    ...
    fun updateChipGroupData(chipGroupId: Int, newSelection: List<String>) {
        when (chipGroupId) {
            R.id.chip_group_disease_history -> updateChildVisitData(diseaseHistory = newSelection)
