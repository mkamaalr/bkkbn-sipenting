...
import com.bkkbnjabar.sipenting.databinding.FragmentChildRegistration1Binding
...
class ChildRegistrationFragment3 : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentChildRegistration1Binding.inflate(inflater, container, false)
        return binding.root
    }
...
//            }
        ...
        binding.etName.doAfterTextChanged { text -> viewModel.updateChildRegistrationData(name = text.toString()) }
        binding.etNik.doAfterTextChanged { text -> viewModel.updateChildRegistrationData(nik = text.toString()) }
        binding.etPhoneNumber.doAfterTextChanged { text -> viewModel.updateChildRegistrationData(phoneNumber = text.toString()) }
        binding.etFullAddress.doAfterTextChanged { text -> viewModel.updateChildRegistrationData(fullAddress = text.toString()) }
        binding.etDateOfBirth.doAfterTextChanged { text -> viewModel.updateChildRegistrationData(dateOfBirth = text.toString()) }
        ...
        binding.etRw.setOnItemClickListener { adapterView, _, position, _ ->
            ...
            viewModel.updateChildRegistrationData(rwId = selectedRw.id, rwName = selectedRw.name, rtId = null, rtName = null)
        ...
        binding.etRt.setOnItemClickListener { adapterView, _, position, _ ->
            ...
            viewModel.updateChildRegistrationData(rtId = selectedRt.id, rtName = selectedRt.name)
        }
    ...
    private fun observeViewModel() {
        ...
        viewModel.currentChild.observe(viewLifecycleOwner) { data ->
    ...
    private fun updateUiWithData(data: ChildData) {
    ...
    private fun saveUIToViewModel() {
        ...
        viewModel.updateChildRegistrationData(
