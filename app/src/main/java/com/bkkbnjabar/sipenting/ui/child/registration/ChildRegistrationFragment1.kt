...
//            }
        ...
        binding.etName.doAfterTextChanged { text -> viewModel.updateChildData(name = text.toString()) }
        binding.etNik.doAfterTextChanged { text -> viewModel.updateChildData(nik = text.toString()) }
        binding.etPhoneNumber.doAfterTextChanged { text -> viewModel.updateChildData(phoneNumber = text.toString()) }
        binding.etFullAddress.doAfterTextChanged { text -> viewModel.updateChildData(fullAddress = text.toString()) }
        binding.etDateOfBirth.doAfterTextChanged { text -> viewModel.updateChildData(dateOfBirth = text.toString()) }
        ...
        binding.etRw.setOnItemClickListener { adapterView, _, position, _ ->
            ...
            viewModel.updateChildData(rwId = selectedRw.id, rwName = selectedRw.name, rtId = null, rtName = null)
        ...
        binding.etRt.setOnItemClickListener { adapterView, _, position, _ ->
            ...
            viewModel.updateChildData(rtId = selectedRt.id, rtName = selectedRt.name)
        }
    ...
    private fun updateUiWithData(data: ChildRegistrationData) {
